...
# Build indicators
ted_threshold = 0.593827727 + 3 * 0.560750914  # Using mean and stdev of ted in jan 2007, calculated in excel
...
ted["Panic_Signal"] = ted_threshold  # If we rather the threshold be a constant line

for df, rename in [(vix, "vix"), (dji, "dji"), (gspc, "gspc"), (aig, "aig"), (citi, "citi"), (jpm, "jpm")]:
    ...
    df["Normalized_Close"] = df["Close"] / df["Close"].loc[
        (dm.string_to_datetime("2008-01-01") <= df["Date"]) & (df["Date"] <= dm.string_to_datetime("2008-01-31"))
    ].iloc[0]
...
vertical_lines = [
    {
        ...
        "x": dm.string_to_unix("2008-09-15")
    }
]
# To focus on specific period
vis.focus("2008-01-01", "2008-10-31")
vis.plot_indicators(title="Global Financial Crisis (01/01/2008 - 31/10/2008)", graphs=graphs, v_lines=vertical_lines)